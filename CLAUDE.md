...
- [x] Fase 2 (secondo blocco) — 2026-07-06: **salvataggio/condivisione giri**. SQLite (better-sqlite3, file `data/bikemaps.db`, WAL) nell'API: POST/GET/DELETE `/api/routes` con validazione JSON-schema fastify, id `randomBytes(6).base64url`. Si salva la RICHIESTA (points/mode/profilo/loopKm/seed) + path completo + statistiche; il link `/?r=ID` ripristina i parametri e ricalcola (anelli riproducibili grazie al seed). UI: 💾 Salva giro (nome + link in clipboard), 📁 I miei giri (apri/link/elimina). Dev: Vite proxa anche `/api`→3000, serve l'API attiva. Docker: base `node:22-slim` (better-sqlite3 vuole glibc), BM_DB su volume data/. Schema piatto portabile a PostGIS per il multi-utente. Verificato e2e in preview.
